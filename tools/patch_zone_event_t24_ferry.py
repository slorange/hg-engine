#!/usr/bin/env python3
"""Add Cianwood → Route 40 ferry fisherman + Lapras to zone_event member 072."""
...
FERRY_OBJECT_ID = 12
LAPRAS_OBJECT_ID = 13
# Face west toward town / player on the beach (pret: DIR_WEST = 2).
FACING_WEST = 2
...
def patch_member(data: bytearray) -> None:
    ...
    if removed:
        print(f"removed {removed} existing Cianwood ferry object(s)")
...
def main(argv: list[str]) -> int:
    if len(argv) != 2:
        print(f"usage: {argv[0]} <build/a032/2_072>", file=sys.stderr)
        return 1
    ...
    print(
        f"installed Cianwood ferry at ({FERRY_X}, {FERRY_Z}) + Lapras at ({LAPRAS_X}, {LAPRAS_Z}) "
        f"in {target}"
    )
    return 0
