#!/usr/bin/env python3
"""Populate build/a032 from clean zone_event extracted from rom.nds."""
...
ROM = ROOT / "rom.nds"
VANILLA_DIR = ROOT / "build/a032_vanilla"
VANILLA_NARC = ROOT / "build/vanilla_rom_root/a/0/3/2"


def ensure_vanilla_zone_event() -> None:
    if not ROM.is_file():
...
def main(argv: list[str]) -> int:
    ...
    out_dir = Path(argv[1])
    ensure_vanilla_zone_event()

    if out_dir.exists():
        shutil.rmtree(out_dir)
    shutil.copytree(VANILLA_DIR, out_dir)

    print(f"seeded {out_dir} from rom.nds zone_event ({len(list(out_dir.glob('2_*')))} members)")
    return 0
