#!/usr/bin/env python3
"""Add Route 31 → Route 45 Dark Cave ferry hiker to zone_event member 032."""
...
def patch_member(data: bytearray) -> None:
    ...
    if removed:
        print(f"removed {removed} existing Route 31 ferry object(s)")
...
def main(argv: list[str]) -> int:
    if len(argv) != 2:
        print(f"usage: {argv[0]} <build/a032/2_032>", file=sys.stderr)
        return 1
    ...
    print(f"installed Route 31 ferry at ({FERRY_X}, {FERRY_Z}) in {target}")
    return 0
