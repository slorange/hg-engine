#!/usr/bin/env python3
"""Add Blackthorn → Route 44 ferry hiker + Piloswine to zone_event member 086."""
...
def patch_member(data: bytearray) -> None:
    ...
    if removed:
        print(f"removed {removed} existing Blackthorn ferry object(s)")
...
def main(argv: list[str]) -> int:
    if len(argv) != 2:
        print(f"usage: {argv[0]} <build/a032/2_086>", file=sys.stderr)
        return 1
    ...
    if target.name != "2_086":
        print(f"warning: expected zone_event 2_086 (Blackthorn), got {target.name}", file=sys.stderr)

    data = bytearray(target.read_bytes())
    patch_member(data)
    target.write_bytes(data)
    print(
        f"installed Blackthorn ferry at ({FERRY_X}, {FERRY_Z}) + "
