#!/usr/bin/env python3
"""Patch Ecruteak Gym Morty (scr_seq 922): badge-count HM before TM grant."""
...
def load_vanilla_member() -> bytearray:
    ...
    return bytearray(VANILLA_MEMBER.read_bytes())
SLOT1_ASM = Path(__file__).resolve().parents[1] / "armips/scr_seq/scr_seq_morty_gym_slot1.s"
SLOT1_BIN = Path(__file__).resolve().parents[1] / "build/morty_gym_slot1.bin"
...
def main(argv: list[str]) -> int:
    ...
    print(
        f"patched Morty gym slot 1 in {target} ({len(data)} bytes, "
