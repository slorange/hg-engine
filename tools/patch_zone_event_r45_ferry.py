#!/usr/bin/env python3
"""Add Route 45 → Route 31 Dark Cave ferry hiker to zone_event member 044."""
...
def patch_member(data: bytearray) -> None:
    ...
    ferry_ids = {FERRY_OBJECT_ID, QUAGSIRE_OBJECT_ID}
    kept = [obj for obj in objects if object_id(obj) not in ferry_ids]
    removed = len(objects) - len(kept)
    if removed:
        print(f"removed {removed} existing Route 45 ferry object(s)")
...
def main(argv: list[str]) -> int:
    if len(argv) != 2:
        print(f"usage: {argv[0]} <build/a032/2_044>", file=sys.stderr)
        return 1
    ...
    print(f"installed Route 45 ferry at ({FERRY_X}, {FERRY_Z}) in {target}")
    return 0
