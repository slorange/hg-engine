...
ASFLAGS =  -I$(shell pwd)/asm/include -I$(shell pwd)/include -include $(shell pwd)/include/config.h -mthumb -mcpu=arm946e-s -mtune=arm946e-s
...
all: $(OUTPUT) $(OVERLAY_OUTPUTS) $(WILD_LEVEL_CAPS_C) $(LEVEL_UP_EVO_TABLES_C) $(SYNTHETIC_EVO_EDGES_C) $(TOOLS) $(BASE)/arm9.bin
